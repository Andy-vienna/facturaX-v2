...
public class MainWindow extends JFrame {
    ...
    private JPanel status, pageWorkTime, pageWT, pageEmployee, pageEM;
    private JLabel lblState;
    private JComboBox<String> cmbMonth; private JComboBox<String> cmbUser;
    private JComboBox<String> cmbEmployee;
    
    private List<String> userList = null;
    private int monthWT = LocalDate.now().getMonthValue();
    private int userWT = 0; private int userEM = 0; private int activeScreen = 0;
    ...
    private MainWindow() {
		...
		doWorkTimePanel(); // WorkTimePanel bauen
		doEmployeePanel(); // EmployeePanel bauen
        buildStatusBar();  // Statuszeile bauen
        doSwitchScreen(1); // initialen Screen setzen
    }
    ...
    private void buildMenuBar() {
        ...
        JMenuItem arbeitszeit = new JMenuItem("Arbeitszeit", MenuIcon.INFO.icon());
        JMenuItem mitarbeiter = new JMenuItem("Mitarbeiter", MenuIcon.INFO.icon());

        menu1.add(exit);
        menu2.add(aktualisieren);
        menu3.add(arbeitszeit);
        menu3.add(mitarbeiter);
        ...
        arbeitszeit.addActionListener(_ -> doSwitchScreen(1));
        mitarbeiter.addActionListener(_ -> doSwitchScreen(2));
    ...
    private void doWorkTimePanel() {
    	...
    	String[] monthName = { "", "Januar", "Februar", "März", "April", "Mai", "Juni",
        ...
        cmbMonth = new JComboBox<>(monthName);
        cmbMonth.setFont(new Font("Arial", Font.BOLD, 12));
    	...
    	cmbMonth.addActionListener(_ -> changeDisplayedWorkTime());
    	cmbUser.addActionListener(_ -> changeDisplayedWorkTime());
    	
    	cmbMonth.setSelectedIndex(monthWT); // aktuellen Monat laden
    	cmbUser.setSelectedIndex(userWT);
    }
    ...
    private void doSwitchScreen(int screen) {
    	
    	pageWorkTime.removeAll(); pageEmployee.removeAll();
    	pageWT.remove(pageWorkTime); pageEM.remove(pageEmployee);

    	pageWT.removeAll(); pageEM.removeAll();
    	this.remove(pageWT); this.remove(pageEM);
    ...
    private void changeDisplayedWorkTime() {
		...
		if (cmbMonth.getSelectedIndex() == 0 || cmbUser.getSelectedIndex() == 0) {
			pageWorkTime.revalidate(); pageWorkTime.repaint();
			return;
		}
		String inhalt = cmbMonth.getSelectedItem().toString();
